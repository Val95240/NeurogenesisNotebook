...
def get_dataset():
    dataset = IrisDataset(file='dataset/iris.csv', normalize=True)
    idx = np.random.permutation(dataset.vectors.shape[0])
    return dataset.vectors[idx], dataset.labels[idx]
